...
(function () {
  ...
  function hideLoader(img) {
    var loader =
      img.parentElement && img.parentElement.querySelector(".loader");
  ...
  function loadIfVisible(img, margin) {
    ...
    return (
      rect.bottom >= -margin &&
      rect.top <= document.documentElement.clientHeight + margin
    );
  }

  function eagerLoadVisible() {
    document
      .querySelectorAll("img[data-load]:not([data-loaded])")
