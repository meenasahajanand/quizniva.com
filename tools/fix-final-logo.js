...
walk(baseDir, (file) => {
    ...
    // Remove span wrapper around logo img tag and fix the img tag
    content = content.replace(
        /<span[^>]*class="flex-none[^>]*><img alt="lightingport"[^>]*src="[^"]*"[^>]*\/><\/span>/g,
        `<img alt="lightingport" width="150" height="80" style="max-width:150px;height:80px;object-fit:contain;display:block;" src="${logoPath}" />`
    );
    
    // Also fix if there's no span wrapper already
    content = content.replace(
